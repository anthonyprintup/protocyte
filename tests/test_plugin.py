...
def test_generated_header_emits_constants_and_array_storage() -> None:
    ...
    assert header.count("template<class Value>\n  ::protocyte::Status set_blob(const Value &value) noexcept") == 2
    assert "requires(::protocyte::TextPointer<Value>)" in header
    ...
    assert "const auto view = ::protocyte::cstring_byte_span_of(value);" in header
