    ...
    fake_protoc_script.write_text(
        "\n".join(
            [
                ...
                f"configured_output_directory = Path({str(output_directory)!r})",
                "argument_file = Path(sys.argv[1].removeprefix('@'))",
                "arguments = [",
                ...
                "    if argument.startswith('--protocyte_out=')",
                ")",
                "output_value = output_argument.removeprefix('--protocyte_out=')",
                "if ':' in output_value:",
                "    _, output_value = output_value.split(':', 1)",
                "protoc_output_directory = Path(output_value)",
                "def staged_path(raw_path: str) -> Path:",
                "    return protoc_output_directory / Path(raw_path).relative_to(configured_output_directory)",
                "for proto_name, paths in outputs.items():",
                "    if proto_name not in arguments:",
                "        continue",
                "    for raw_path in paths:",
                "        output = staged_path(raw_path)",
...
def _make_fake_protoc_fail_in_build(
    ...
    *,
    ...
    # Exercise error cleanup after a plugin has already produced plausible
    # staged headers, sources, and runtime files.
    instrumented += "\n" + "\n".join(failure_lines) + "\n"
...
def _write_fault_instrumented_generation_script(script_dir: Path) -> Path:
    ...
    output_publication_anchor = '    file(RENAME "${staged_generation_output}" "${generation_output}" NO_REPLACE RESULT output_publish_result)\n'
    output_publication_instrumentation = (
        "    if(\n"
        "        DEFINED PROTOCYTE_TEST_FAIL_OUTPUT_PUBLICATION_INDEX\n"
        "        AND generation_output_index EQUAL PROTOCYTE_TEST_FAIL_OUTPUT_PUBLICATION_INDEX\n"
        "    )\n"
        '        set(output_publish_result "injected output publication failure")\n'
        "    else()\n" + output_publication_anchor + "    endif()\n"
    )
    assert generation_source.count(output_publication_anchor) == 1
    generation_source = generation_source.replace(
    ...
    release_phase_anchor = "        set(transaction_is_releasing TRUE)\n"
    release_phase_instrumentation = (
        release_phase_anchor
        + "        if(DEFINED PROTOCYTE_TEST_ABORT_AFTER_RECOVERY_RELEASE_PHASE)\n"
        + '            message(FATAL_ERROR "injected recovery abort after owner-release phase")\n'
        + "        endif()\n"
    )
    assert recovery_source.count(release_phase_anchor) == 1
    recovery_source = recovery_source.replace(
        release_phase_anchor, release_phase_instrumentation
    )
    ...
    instrument_recovery_action(
        ...
        "protocyte_test_recovery_witness_removal_count",
        "PROTOCYTE_TEST_ABORT_AFTER_RECOVERY_WITNESS_REMOVAL",
        "owner witness removal",
    )
...
def _run_direct_owner_generation(
    ...
    *extra_arguments: str,
    ...
    argument_file.write_text(
        "\n".join(
            [
                f"--protocyte_out=format=off:{staging_directory / 'generated'}",
                "demo_0.proto",
                "",
            ]
        ),
    ...
    return subprocess.run(
        [
            ...
            f"-DSTAGING_OUTPUT_DIRECTORY={staging_directory}",
...
def _path_tree_snapshot(path: Path) -> dict[str, bytes | str]:
...
def test_output_publication_failure_restores_prior_outputs_and_releases_ownership(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    failed_build_dir = tmp_path / "failed-build"
    ...
    failed = _run_direct_owner_generation(
        ...
        output_directory,
        instrumented_script,
        "-DPROTOCYTE_TEST_FAIL_OUTPUT_PUBLICATION_INDEX=1",
    )

    assert failed.returncode != 0
    assert "injected output publication failure" in failed.stdout + failed.stderr
...
def test_generation_transaction_committed_journal_requires_every_planned_claim(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    owner_build_dir = tmp_path / "owner-build"
    ...
    transaction = next(
        (tmp_path / "output-locks").glob(".protocyte-generation-*.committed")
    )
    removed_claim = next((tmp_path / "output-locks").glob("*.owner"))
    removed_claim.unlink()
    ...
    before_retry = {
        ...
        "staging": _path_tree_snapshot(staging_directory),
    ...
    assert _path_tree_snapshot(staging_directory) == before_retry["staging"]
...
def test_generation_transaction_valid_committed_journal_is_cleaned(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    owner_build_dir = tmp_path / "owner-build"
    ...
    assert interrupted.returncode != 0
    assert list((tmp_path / "output-locks").glob(".protocyte-generation-*.committed"))
...
def test_generation_transaction_rejects_tampered_owner_list_before_rollback(
    ...
    if tamper_owner_list == "duplicate":
        ...
        transaction_lines[owner_line_indexes[0]] = "owner-key=unexpected;owner"
...
def test_generation_transaction_ignores_forged_zero_owner_staging_journal(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    build_dir = tmp_path / "build"
    ...
    first = _run_direct_owner_generation(
    ...
    (staging_directory / "publication.transaction.active").write_text(
        "version=2\nowner-count=0\nforged=staging\n", encoding="utf-8"
    )
...
@pytest.mark.parametrize("tamper", ["directory", "bytes"])
def test_generation_transaction_rejects_late_backup_tamper_before_rollback(
    tmp_path: Path,
    tamper: str,
) -> None:
    source_dir = tmp_path / "project"
    ...
    assert interrupted.returncode != 0
    staging_directory = next(tmp_path.glob(".protocyte-generation-staging-*"))
    backup_root = staging_directory / "backups"
    later_backup = next(
        backup
        for backup in backup_root.rglob("*")
        if backup.is_file()
        and not (output_directory / backup.relative_to(backup_root)).exists()
    )
    if tamper == "directory":
        later_backup.unlink()
        later_backup.mkdir()
    else:
        later_backup.write_bytes(b"forged backup bytes\n")
    before_retry = {
        "outputs": _path_tree_snapshot(output_directory),
        "staging": _path_tree_snapshot(staging_directory),
        "locks": _path_tree_snapshot(tmp_path / "output-locks"),
...
def test_generation_transaction_rejects_published_output_byte_tamper_before_rollback(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    owner_build_dir = tmp_path / "owner-build"
    ...
    assert interrupted.returncode != 0
    published_output = next(
        path for path in output_directory.rglob("*") if path.is_file()
    )
    published_output.write_bytes(b"forged generated output bytes\n")
    staging_directory = next(tmp_path.glob(".protocyte-generation-staging-*"))
    before_retry = {
        "outputs": _path_tree_snapshot(output_directory),
        "staging": _path_tree_snapshot(staging_directory),
...
def test_generation_transaction_rejects_old_authoritative_journal_without_mutation(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    owner_build_dir = tmp_path / "owner-build"
...
def test_generation_transaction_rejects_truncated_journal_before_rollback(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    owner_build_dir = tmp_path / "owner-build"
    ...
    assert transaction_bytes.endswith(b"\n")
    transaction.write_bytes(transaction_bytes[:-7])
    staging_directory = next(tmp_path.glob(".protocyte-generation-staging-*"))
    before_retry = {
        "outputs": _path_tree_snapshot(output_directory),
        "staging": _path_tree_snapshot(staging_directory),
...
def test_generation_transaction_journal_contains_relative_plan_and_hashes_only(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    owner_build_dir = tmp_path / "owner-build"
    ...
    transaction_content = transaction.read_text(encoding="utf-8")
    assert tmp_path.as_posix() not in transaction_content
    assert "output-relative-hex=" in transaction_content
    assert "owner-key=" in transaction_content
    assert "owner-hex=" not in transaction_content
...
def test_generation_transaction_rejects_tampered_owner_release_witness(
    tmp_path: Path,
    tamper: str,
) -> None:
    source_dir = tmp_path / "project"
    ...
    transaction = next(
        (tmp_path / "output-locks").glob(".protocyte-generation-*.releasing")
    )
    transaction_lines = transaction.read_text(encoding="utf-8").splitlines()
    transaction_id = next(
...
def test_generation_transaction_recovers_after_owner_release_phase_rename(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    owner_build_dir = tmp_path / "owner-build"
    ...
    assert interrupted.returncode != 0
    phase_interrupted = _run_direct_owner_generation(
        source_dir,
        owner_build_dir,
        output_directory,
        instrumented_script,
    ...
    assert phase_interrupted.returncode != 0
    assert "injected recovery abort after owner-release phase" in (
        phase_interrupted.stdout + phase_interrupted.stderr
    )
...
def test_generation_transaction_recovers_after_owner_witness_removal(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    owner_build_dir = tmp_path / "owner-build"
    ...
    assert interrupted.returncode != 0
    witness_interrupted = _run_direct_owner_generation(
        source_dir,
        owner_build_dir,
        output_directory,
        instrumented_script,
        "-DPROTOCYTE_TEST_ABORT_AFTER_RECOVERY_WITNESS_REMOVAL=1",
        reuse_build=True,
    )
    assert witness_interrupted.returncode != 0
    assert "injected recovery abort after owner witness removal" in (
        witness_interrupted.stdout + witness_interrupted.stderr
    )
    root_owner, _ = _out_dir_owner_record_paths(output_directory)
...
def test_generation_transaction_rejects_releasing_journal_without_witness_with_remaining_claims(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    owner_build_dir = tmp_path / "owner-build"
...
def test_generation_transaction_rejects_releasing_journal_before_output_restoration(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    owner_build_dir = tmp_path / "owner-build"
    ...
    assert interrupted.returncode != 0
    active = next((tmp_path / "output-locks").glob(".protocyte-generation-*.active"))
    releasing = active.with_name(active.name.removesuffix(".active") + ".releasing")
    active.replace(releasing)
    staging_directory = next(tmp_path.glob(".protocyte-generation-staging-*"))
    before_retry = {
        "outputs": _path_tree_snapshot(output_directory),
        "staging": _path_tree_snapshot(staging_directory),
...
def test_generation_transaction_accepts_legacy_v1_retained_owners(
    ...
    owner_marker, _ = _out_dir_owner_record_paths(output_directory)
    owner_records = [owner_marker, *(tmp_path / "output-locks").glob("*.owner")]
    legacy_record = (
        f"version=1\nbuild-tree-sha256={_build_tree_owner_hash(build_dir)}\n"
    )
    for owner_record in owner_records:
        owner_record.write_text(legacy_record, encoding="utf-8")

    regenerated = _run_direct_owner_generation(
        source_dir,
        build_dir,
        output_directory,
        generation_script,
...
def test_generation_transaction_recovers_incremental_mixed_owner_epochs(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "project"
    build_dir = tmp_path / "build"
    ...
    _make_direct_owner_fake_protoc_write_outputs(source_dir, output_directory, 1)
    first = _run_direct_owner_generation(
        source_dir, build_dir, output_directory, generation_script
    )
    assert first.returncode == 0, first.stdout + first.stderr
    retained_before = _out_dir_snapshot(output_directory)

    _make_direct_owner_fake_protoc_write_outputs(source_dir, output_directory, 2)
    instrumented_script = _write_fault_instrumented_generation_script(
        tmp_path / "instrumented-cmake"
    )
    interrupted = _run_direct_owner_generation(
    ...
    assert interrupted.returncode != 0
    _make_fake_protoc_fail_in_build(source_dir, build_dir)
    recovered = _run_direct_owner_generation(
        source_dir,
        build_dir,
        output_directory,
    ...
    assert recovered.returncode != 0
    assert "simulated protoc failure" in recovered.stdout + recovered.stderr
    assert _out_dir_snapshot(output_directory) == retained_before
    assert not list((tmp_path / "output-locks").glob(".protocyte-generation-*.active"))
    assert len(list((tmp_path / "output-locks").glob("*.owner"))) == 2
...
def test_generation_transaction_publication_scales_near_linearly(
    tmp_path: Path,
) -> None:
    """Keep fake-protoc publication below the old journal-rewrite cliff."""
    ...
    for descriptor_count in (16, 64):
        source_dir = tmp_path / f"project-{descriptor_count}"
        build_dir = tmp_path / f"build-{descriptor_count}"
        output_directory = tmp_path / f"generated-{descriptor_count}"
        _write_out_dir_owner_project(source_dir, output_directory)
        _make_direct_owner_fake_protoc_write_outputs(
            source_dir, output_directory, descriptor_count
        )
        for phase in ("first", "repeat"):
            ...
            result = _run_direct_owner_generation(
                source_dir,
                build_dir,
                output_directory,
                generation_script,
                reuse_build=phase == "repeat",
                descriptor_count=descriptor_count,
            )
            timings[descriptor_count, phase] = time.perf_counter() - started
            assert result.returncode == 0, result.stdout + result.stderr

    # The absolute limit leaves headroom for loaded CI hosts; the ratio rejects
    # reintroducing per-output whole-plan rewrites while allowing small fixed
    # process/CMake startup costs to dominate the 16-descriptor sample.
    for phase in ("first", "repeat"):
        small = timings[16, phase]
        large = timings[64, phase]
        assert large < 12.0, (phase, small, large)
        assert large < small * 6.0 + 2.0, (phase, small, large)
...
def test_generation_lock_wrapper_preserves_protoc_failure_diagnostics(
    ...
    output_directory = tmp_path / "generated"
    staging_directory = (
        output_directory.parent / ".protocyte-generation-staging-failure-diagnostics"
    )
...
def test_generation_timeout_kills_wrapper_descendants_without_publishing_outputs(
    ...
    assert (
        "Staged generation output was discarded before ownership was published"
        in output
    )
    marker, _ = _out_dir_owner_record_paths(output_directory)
    assert not marker.exists()
    ...
    (source_dir / "fake-protoc.py").write_text(original_fake_protoc, encoding="utf-8")
    retried = _build_out_dir_owner_project(timed_out_build_dir)
    assert retried.returncode == 0, retried.stdout + retried.stderr
    assert _committed_owner_build_hash(marker, marker) == (
        _build_tree_owner_hash(timed_out_build_dir)
    )
    assert (output_directory / "demo_0.protocyte.hpp").is_file()
    assert (output_directory / "demo_0.protocyte.cpp").is_file()
