...
namespace protocyte {
    ...
    template<class T, usize Max> struct Array {
        ...
        Array(Array &&other) noexcept {
            for (usize i {}; i < other.size_; ++i) {
                new (ptr(i)) T {protocyte::move(other[i])};
            }
            size_ = other.size_;
            other.clear();
        }
    ...
    protected:
        ...
        const T *ptr(const usize index) const noexcept { return reinterpret_cast<const T *>(&storage_[index * sizeof(T)]); }
    ...
    template<class Reader> Result<i32> read_sint32(Reader &reader) noexcept { return read_zigzag32_scalar<i32>(reader); }

    template<class Reader> Result<i64> read_sint64(Reader &reader) noexcept { return read_zigzag64_scalar<i64>(reader); }
