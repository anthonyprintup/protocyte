...
function(
    ...
    if(transaction_line_count LESS 7)
        return()
    endif()
    list(GET transaction_lines 0 transaction_version)
    list(GET transaction_lines 1 transaction_build_hash_line)
    list(GET transaction_lines 2 transaction_ownership_state_line)
    list(GET transaction_lines 3 transaction_owner_id_line)
    list(GET transaction_lines 4 transaction_owner_witness_line)
    list(GET transaction_lines 5 transaction_owner_count_line)
    if(
        NOT transaction_version STREQUAL "version=2"
    ...
    if(generation_output_count GREATER 0)
        ...
        foreach(generation_output_index RANGE 0 ${last_generation_output_index})
            ...
            math(EXPR transaction_recovery_line_index "${transaction_line_index} + 3")
...
function(_protocyte_recover_generation_transaction out_recovered)
    ...
    foreach(allowed_transaction_owner_identity_key IN LISTS allowed_transaction_owner_identity_keys)
        set("_protocyte_allowed_transaction_owner_${allowed_transaction_owner_identity_key}" TRUE)
    endforeach()
    ...
    foreach(transaction_owner_identity_key IN LISTS transaction_owner_identity_keys)
        if(
            NOT DEFINED "_protocyte_allowed_transaction_owner_${transaction_owner_identity_key}"
        )
            return()
        endif()
    endforeach()
    ...
    if(transaction_ownership_state STREQUAL "commit-pending")
        ...
        set(current_pending_owners)
        set(pending_owner_transaction_ids)
        if(transaction_owner_count GREATER 0)
            ...
            foreach(transaction_owner_index RANGE 0 ${last_transaction_owner_index})
                ...
                if(transaction_owner_status STREQUAL "incomplete")
                    ...
                    set(transaction_owner_status "missing")
                endif()
                if(transaction_owner_status STREQUAL "current")
                    list(APPEND current_pending_owners "${transaction_owner_marker}")
                    list(APPEND pending_owner_transaction_ids "${transaction_owner_id}")
                elseif(NOT transaction_owner_status STREQUAL "missing")
                    return()
                endif()
            endforeach()
        endif()
        list(LENGTH current_pending_owners current_pending_owner_count)
        if(current_pending_owner_count EQUAL 0)
            foreach(transaction_operation_state IN LISTS transaction_operation_states)
        ...
        if(NOT current_pending_owner_count EQUAL transaction_owner_count)
            return()
        endif()
        list(REMOVE_DUPLICATES pending_owner_transaction_ids)
        list(LENGTH pending_owner_transaction_ids pending_owner_transaction_id_count)
        if(NOT pending_owner_transaction_id_count EQUAL 1)
            return()
        endif()
        list(GET pending_owner_transaction_ids 0 transaction_owner_transaction_id)
        _protocyte_generation_transaction_claims_match(
            transaction_claims_match
            transaction_owner_markers
            "${transaction_owner_transaction_id}"
        )
    ...
    if(transaction_owner_count EQUAL 0)
        ...
        set(all_transaction_owners_released TRUE)
        math(EXPR last_transaction_owner_index "${transaction_owner_count} - 1")
        foreach(transaction_owner_index RANGE 0 ${last_transaction_owner_index})
            list(GET transaction_owner_markers ${transaction_owner_index} transaction_owner_marker)
            list(GET transaction_owner_release_states ${transaction_owner_index} transaction_owner_release_state)
            ...
            if(transaction_owner_release_state STREQUAL "unreleased")
                ...
                set(all_transaction_owners_released FALSE)
        ...
        elseif(transaction_owner_witness_state STREQUAL "remove-pending")
            if(NOT all_transaction_owners_released)
                return()
            endif()
        elseif(NOT transaction_owner_witness_state STREQUAL "removed")
            return()
        endif()
    endif()
    if(generation_output_count GREATER 0)
    ...
    if(transaction_owner_count GREATER 0)
        ...
        foreach(transaction_owner_index RANGE 0 ${last_transaction_owner_index})
            list(GET transaction_owner_markers ${transaction_owner_index} transaction_owner_marker)
            list(GET transaction_owner_release_states ${transaction_owner_index} transaction_owner_release_state)
            if(transaction_owner_release_state STREQUAL "released")
                continue()
            endif()
            _protocyte_set_generation_transaction_list_item(
    ...
    if(NOT transaction_owner_witness_state STREQUAL "removed")
