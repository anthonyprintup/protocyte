...
`RUNTIME_PREFIX` and the lower-level `INCLUDE_PREFIX` value are virtual include
directories, not arbitrary host filesystem paths. They must be normalized,
relative, `/`-separated paths and must not contain `.`/`..` segments, Windows
device names, or characters unsafe in generated includes.
